...
SELECT
    ...
    CASE
        WHEN enrollee_status_description = 'Enrolled' THEN 1
        WHEN enrollee_status_description = 'Cancelled' THEN 2
        WHEN enrollee_status_description = 'Terminated' THEN 3
    END AS status_Id,

    CASE
        WHEN enrollee_status_description = 'Enrolled' THEN 'CONFIRM'
        WHEN enrollee_status_description = 'Cancelled' THEN 'CANCEL'
        WHEN enrollee_status_description = 'Terminated' THEN 'TERM'
    END AS enrolleeStatus,
...
WHERE coverage_year = 2025
  AND MONTH(GAA_Load_Datetime) = 6
  AND enrollee_status_description IN ('Enrolled', 'Cancelled', 'Terminated')
